...
## Energy consumption ##
The following figure visualizes energy consumption of different operation states. The numbers at the measuring point equal to the duration.

```{r echo=FALSE,warning=FALSE}
# install required packages
list.of.packages <- c("openxlsx","zip","tidyverse")
...
energymeasurement <- read.xlsx("Energiemessung.xlsx", sheet="Tabelle1")

energymeasurement <- as_tibble(energymeasurement)

energymeasurement <- energymeasurement %>% rename(action=Aktion, 
                              flash=Flash, 
...
energymeasurement %>% ggplot() +
  aes(x=samplingrate_hz, y=energy_mJ, color=flash) +
  geom_line() +
  geom_point(aes(shape=action), size=3) +
  geom_text(aes(label=duration_s), nudge_y=70, color="black") +
  theme_bw() +
  ylab("energy / mJ") +
  xlab("samplingrate / Hz")
```
