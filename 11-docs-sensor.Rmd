...
The first step in activation is to check if some conditions are fulfilled. The function returns an error code if acceleration logging is already active or if it is called on a sensor which does not include an LIS2DH12. This check is done by calling `find_sensor()` inside of app_sensor.c (2). This function returns the sensor context. The sensor context consists of several information about the sensor. In the next step associates the function `on_fifo_full()` from app_accelerometer_logging.c with the interrupt pin retrieved from the sensor context (3). The last step is to activate FIFO and interrupt generation inside the LIS2DH12. This is done by calling two functions from the sensor context. At least the function pointer to `data_get()` inside the sensor context is replaced by the pointer to the function `lis2dh12_logged_data_get()`. 
...
Inside `fifo_full_handler()` the FIFO from LIS2DH12 is read (5). The values are store in the memory in raw format to be ready to present them to the function `lis2dh12_logged_data_get()` which is important for heartbeat. In parallel the raw values are compacted by removing all unused bits. This is done by the functions `pack8/10/12()`. A timestamp is added to the compacted values. These values are handover to the ringbuffer which writes them to flash (6).
...
This function retrieves raw accelerometer values from RAM. The values are parsed and returned inside data. It is called by `app_sensor_get()` inside app_sensor.c if accelerometer logging is active.
...
This function is called when a request to send the last sample is received by GATT/UART. It retrieves the last sample from RAM, does the compacting of the bits by calling `pack8/10/12()` and sends the data to the requester.
...
This function reads raw acceleration values from the registers of LIS2DH12. It is called from the interrupt handler inside app_accelerometer_logging and from `ri_lis2dh12_data_get()` inside this module.
...
This function is used if the acceleration logging is not active. If acceleration logging is active this function is replaced by `lis2dh12_logged_data_get()` inside app_accelerometer_logging.c.
...
| raw_temperature | in | Raw temperature value. If used from app_accelerometer_logging.c this parameter is NULL. |
